...
Future<void> scanLibrary(BuildStep buildStep, String libPackageName) async {
  try {
...
//    e.typeParameters.forEach((element) {
//      parseAddImport(element.bound);
//    });
//  }

  bool scanConstructors = !annotation
      .peek('scanConstructors')
      .isNull &&
      annotation
          .peek('scanConstructors')
          .boolValue;
  ...
  annotation
      .peek('scanConstructorsUsedBlackList')
      .isNull
      ? true
      : annotation
      .peek('scanConstructorsUsedBlackList')
  ...
  bool scanFieldsUsedBlackList =
      !annotation
          .peek('scanFieldsUsedBlackList')
          .isNull &&
          annotation
              .peek('scanFieldsUsedBlackList')
              .boolValue;
  bool scanSuperFields = !annotation
      .peek('scanSuperFields')
      .isNull &&
      annotation
          .peek('scanSuperFields')
          .boolValue;

  bool scanMethods = !annotation
      .peek('scanMethods')
      .isNull &&
      annotation
          .peek('scanMethods')
  ...
  bool scanMethodsUsedBlackList =
      !annotation
          .peek('scanMethodsUsedBlackList')
          .isNull &&
          annotation
              .peek('scanMethodsUsedBlackList')
              .boolValue;
  bool scanSuperMethods = !annotation
      .peek('scanSuperMethods')
      .isNull &&
      annotation
          .peek('scanSuperMethods')
          .boolValue;
...
List<GBeanConstructor> _parseGBeanConstructors(ClassElement element,
...
_scanBeanCreator(Element element, ConstantReader annotation) {
  ...
  if (!key.endsWith(".sys.bf.aymtools.dart") &&
      !key.endsWith("beanfactory.sys.aymtools.dart")) {
...
TypeChecker _typeCheckerTypeAdapterAnn = TypeChecker.fromRuntime(TypeAdapter);
TypeChecker _typeCheckerTypeConvert = TypeChecker.fromRuntime(TypeConvert);

List<GBean> scanTypeAdapter() =>
    beanMap.values
        .where((e) =>
    e.isForAnnotation(_typeCheckerTypeAdapterAnn) &&
        e.isAssignableFrom(_typeCheckerTypeConvert))
        .toList();

TypeChecker _typeCheckerInitializerBase =
TypeChecker.fromRuntime(FactoryInitializer);
TypeChecker _typeCheckerInitializerAnn =
TypeChecker.fromRuntime(OnFactoryInitializer);

List<GBean> scanInitializer() =>
    beanMap.values
        .where((e) =>
    e.isForAnnotation(_typeCheckerInitializerAnn) &&
        e.isAssignableFrom(_typeCheckerInitializerBase))
        .toList();
